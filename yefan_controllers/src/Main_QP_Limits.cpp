...
//    fk_req.q = std::vector<double>(q_desired.data(), q_desired.data() + q_desired.size());
//    ros::service::call("/yefan/srv/fk", fk_req, fk_res);
//    Eigen::Matrix<double, 4, 4, Eigen::RowMajor> T_FK_sin(fk_res.T.data());
//    std::cout << "T_sensor: " << T_FK_sin << std::endl;
//    T = T_FK_sin;
...
//    file_x << abs_t << " " << q_sensor.transpose() << tau_output_QP.transpose() << std::endl;
